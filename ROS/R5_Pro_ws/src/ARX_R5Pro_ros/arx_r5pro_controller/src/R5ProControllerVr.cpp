...
namespace arx
{
    ...
    {
        // 创建发布器
        joint_state_publisher_ = nh.advertise<arm_control::PosCmd>("/l5pro_status", 10);
        // 创建订阅器
        joint_state_subscriber_ = nh.subscribe<arm_control::PosCmd>(
            "/ARX_VR_L", 10, &R5proController::CmdCallback, this);
        // 定时器，用于发布关节信息

        timer_ = nh.createTimer(ros::Duration(0.01), &R5proController::PubState, this);
        l5_pro_ineterfaces_ptr_ = std::make_shared<r5::InterfacesThread>("can0", 0);
    }

    void R5proController::CmdCallback(const arm_control::PosCmd::ConstPtr& msg)
    {
        double input[6] = {msg->x, msg->y, msg->z, msg->roll, msg->pitch, msg->yaw};
        Eigen::Isometry3d transform = solve::Xyzrpy2Isometry(input);

        l5_pro_ineterfaces_ptr_->setEndPose(transform);

        l5_pro_ineterfaces_ptr_->setArmStatus(4);

        l5_pro_ineterfaces_ptr_->setCatch(msg->gripper);
    }
    ...
    void R5proController::PubState(const ros::TimerEvent&)
    {
        ...
        Eigen::Isometry3d transform = l5_pro_ineterfaces_ptr_->getEndPose();
