...
namespace arx::r5
{
    R5ProController::R5ProController(ros::NodeHandle nh)
    {
        ...
        if (arm_control_type == "normal")
        {
            ...
            joint_state_subscriber_ = nh.subscribe<arx5_arm_msg::RobotCmd>(
                sub_topic_name, 10, &R5ProController::CmdCallback, this);
            // 定时器，用于发布关节信息
        ...
        else if (arm_control_type == "vr_slave")
        {
            ...
            joint_state_subscriber_ = nh.subscribe<arm_control::PosCmd>(
                sub_topic_name, 10, &R5ProController::VrCmdCallback, this);
            timer_ = nh.createTimer(ros::Duration(0.01), &R5ProController::VrPubState, this);
        }
        ...
        else if (arm_control_type == "aloha_slave")
        {
            ...
            joint_state_subscriber_ = nh.subscribe<arx5_arm_msg::RobotStatus>(
                sub_topic_name, 10, &R5ProController::FollowCmdCallback, this);
            timer_ = nh.createTimer(ros::Duration(0.01), &R5ProController::PubState, this);
        }
    }

    void R5ProController::CmdCallback(const arx5_arm_msg::RobotCmd::ConstPtr &msg)
    {
    ...
    void R5ProController::VrCmdCallback(const arm_control::PosCmd::ConstPtr &msg)
    {
    ...
    void R5ProController::VrPubState(const ros::TimerEvent &)
    {
    ...
    void R5ProController::FollowCmdCallback(const arx5_arm_msg::RobotStatus::ConstPtr &msg)
    {
